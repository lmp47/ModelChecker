...
to find a (possibly incomplete) assignment whose current-state literals give a CTI cube.
...
with $0 \geq j < i$, $(s, j)$ has already been fulfilled.
...
\verb.(1, 0, [prop]). in it, and the initial frame. \verb,prove', then invokes \verb,proveObligations,,
...
enqueues obligation $(s,j)$ with a rank that gives it a priority such tht it occurs after all other
...
a proof obligation for proving the negated CTI a
...
%profiling consistently reveals that ...
...
examples (i.e. examples that require finding counterexamples).

\subsection{Propagation}

As expected, removing subsumed clauses results in a smaller number of literals per clause, 
