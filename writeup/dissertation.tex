...
AND gates, which are always binary,
are specified by giving the indices that represent the positive literal
...
as follows (the comments to the right are not part of the specification):
\begin{verbatim}
aag 3 0 2 1 1
...
Both the \verb,Parser.AigerParser, module, which implements the parser in Haskell, and
the \verb,Parser.AigerTools, module, which calls the Aiger Utilities' parser's functions,
convert the AIGER file into the \verb,Model, data structure in \verb,Parser.AigModel,,
...
Consider a given \verb,Parser.AigModel.Latch, \verb.[l, n, init]. representing the latch with
output variable $l$ (represented by \verb,l,), next-state $n$ (represented
by \verb,n,) taken from the set of Booleans and literals, and initial value (represented by \verb,init,)
...
Consider a \verb,Parser.AigModel.And,, of the form \verb.[out, in1, in2]., representing the AND gate
with output variable $a$ (represented by \verb,out,), and inputs $i_1$ and $i_2$ (represented by \verb,in1,
...
$a' \Leftrightarrow i_1' \wedge i_2'$. To achieve the necessary logical equivalence
for the next-state clauses, the generated current-state clauses are primed.
...
\Fn{${\it prove'}(M,P,F_k,[F_0,\ldots,F_{k - 1}])$}{
  ...
  $({\it result}, [G_0,\ldots,G_{k - 1}], G_k)$ = {\it proveNegCTI}$((i,x,I,T),{\rm cti},k - 1)$ \label{recstr:proveNegCTI}} 
  {
  \If{\it result} {
...
(line \ref{recstr:proveNegCTI})
does not correspond just to the {\it proveNegCTI} algorithm in Figure \ref{overview} but
rather to the while loop that contains that function.
...
{\it proveNegCTI} on line \ref{recstr:proveNegCTI} in Figure \ref{recstr} returns not only
the result
