...
For example, type checking is a widely-used form of formal methods that
proves that programs are well-typed.
Formal verification is a form of formal methods that
that focuses on proving properties of
systems.
...
The preparation for the project involved learning
...
All the code for the project, with the exception of the C wrapper for the
...
as in Standard ML, using \verb,\, to bind variables analagously to Standard
...
was used for managing the project's code, and the Git repository
hosting service GitHub \cite{github} was used to
keep backups.
...
acquisition of project dependencies (e.g., MiniSat can be obtained by running
\verb,git submodule init, after running \verb,git clone, to clone the repository).}

\paragraph{Haddock}{
The commonly-used Haddock \cite{haddock} documentation tool for Haskell was used to generate documentation
...
Symbolic model checkers rely on representing the underlying system as
...
The models that \emph{MC} accepts as input
...
The AIGER format has an ASCII and a binary version; either
...
I describe how variables are represented in all AIGER formats
...
is used to check whether the property $P$ holds in the next
...
The call to {\it nextCTI} in line \ref{CTI} finds the CTI
...
$F_k$ is too loose an approximation, similarly to why a consecution
...
their inductive generalization of CTIs. The variants and the
...
calls the MiniSat C++ function. For instance, the following function
...
The Haskell FFI is then used to provide a wrapper function that calls
the C function:
\begin{lstlisting}
...
The information kept in a \verb,Result, Haskell datatype is taken directly from the
\verb,result, returned by the C Wrapper functions.
I used the \verb,hsc2hs, preprocessor to help handle pointer offsets
when unmarshalling from the C struct.
Beyond straightforward unmarshalling, some additional work
...
transition systems) and frames in the backend.
...
which serves as the representation of the hardware in the backend:
\begin{lstlisting}
...
$\{i, \neg a\}$ and $\{\neg i, a\}$)
are generated, where $i$ is the input that is not a Boolean value.
...
outlining the general structure shared by all the
...
Instead, updated values of frames are returned explicitly from the function call in
...
that are subsumed by other clauses in the list.
...
The simplest method for approximating a MIC attempts to drop each literal in turn and checks
...
Unlike in other variants, in the {\it PriorityQueue} implementation, there is
no distinction between the negation of the safety property $P$ and any other property
needing
to be proved.
The priority queue maintains all the information about which properties need to be
proven, and the main recursive {\it fulfillObligations} function attempts to prove
...
I describe the solving capabilities of the \emph{MC} variants
...
on 14 handwritten examples and 150 examples taken from
...
If an attempt to solve an example took longer than ten minutes, that
...
the outputs of the two parsers and checking that they match. The MiniSat tests
...
The variants could correctly solve all the handwritten examples and 50 HWMCC examples
...
\caption{Benchmark results for the 14 handwritten examples on a log scale.}
\label{eval:time}
\end{figure}
...
Removing subsumed clauses also results in better performance on several examples.
...
most likely because the examples used are too small for the performance benefit of
using CTGs to eliminate more states
to overcome the overheads of finding and proving negated CTGs, which require
...
The early, one-time generalization employed in the \emph{PriorityQueue} variant may
account for its worse perofrmance in spite of the benefits gained from not needing
to rediscover CTIs.
...
remove $(s,i)$ from the
priority queue and enqueue $(s, i + 1)$ before reaching proof obligation
$(\neg p, i + 2)$. It must also then fulfill and remove $(s, i + 1)$ before
...
from being fulfilled ater $(s,i)$ has been fulfilled, then the CTI must be different
from $s$.
...
literals per clause (which suggests larger queries).
}
...
The \emph{MC} implementations do not store as much information in their hardware
models and instead use several SAT-solver queries to extract the necessary literals
from $s$.
