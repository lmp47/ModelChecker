...
The different variants of the model checker were run on fourteen handwritten examples
...
but to solve fifty of the examples taken from the
Hardware Model Checking Competitions within ten minutes as well.
...
For the fourteen handwritten examples and fifty Hardware Model Checking Competition examples that
...
solved six more examples (\verb,counterp0.aig,, \verb,counterp0neg.aig,, \verb,pdtvishuffman7.aig,, 
\verb,pdtvismiim3.aig,, \verb,6s318r.aig,, \verb,srg5ptimo.aig,) than the \emph{Basic} version without timing out.

\subsection{Propagation}

Removing subsumed clauses results in results in better performance for some examples.
While the performance impact that the improvement has is less consistent than the improvement of
...
eliminate more states to overcome the overheads of finding and proving negated CTGs.
...
One of the performance advantages of the {\it PriorityQueue} implementation is that CTIs do not need
...
the {\it CTG} and {\it PriorityQueue} variants of the model checker and examine their effects
on the model checker's performance.
