...
The original aims of the project are to implement the basic
...
The work completed as part of the project includes the implementation
of the major components for completing the original aims of the
project: the AIGER parser, MiniSat interface,
hardware model representation, and basic IC3 implementation. In
addition to this work, several other variants of the IC3 algorithm
were implemented. The different variants can model check small
hardware model examples as well as several larger examples.
...
Unlike formal verification techniques that require user guidance
such as those that employ Hoare Logic or proof assistants, model
checking is fully automated.
...
(described further in section \ref{prep:logic},
...
initial state of the transition system that represents the hardware
and searches for paths of length $k$ from the initial state that violate
the safety property.
...
the algorithm considers sets of states reachable in at most $k$ steps from the
...
there are applications of the algorithm in model checking more
...
%The FAIR model-checking algorithm, which checks $\omega$-regular
%properties, uses a safety model checker such as IC3, and IICTL,
%which checks CTL formulas \cite{hassan12}, uses both a
%safety model checker such as IC3 and a fair-cycle finder such as
%FAIR.

Other work generalizes IC3 to use an underlying SMT solver rather than a
SAT solver. This generalization has been used in the verification of software
\cite{cimatti12}. Results for more recent developments of software verification
...
%This generalization has been used to check control-flow
%graphs of programs \cite{cimatti12}.
%More recently, Johannes Birgmeier, Aaron Bradley, and Georg Weissenbacher
%introduced CTIGAR, a method of abstraction-refinement based on IC3's
...
I successfully implemented not only a basic form of the IC3 algorithm,
...
I discuss the background knowledge about Haskell, the IC3 algorithm, and software
development tools in section \ref{prep}; the implementation of the components of
the model checker and the different variants of the model checker in section
\ref{impl}; and the model-checking capabilities of the implementations in
\ref{eval}. Finally I provide a summary of the completed work and ideas for
future work in section \ref{conc}.
...
The preparation for the project involved learning Haskell (section \ref{prep:haskell}),
distinguishing the main components of the project (section \ref{prep:requirements}),
choosing and learning how to use
...
of hardware models
(section \ref{prep:logic}) and the IC3 algorithm (section \ref{prep:ic3}).
...
is \verb,prove :: Model -> Lit -> Bool,.

%Type constructors
Haskell type constructors and datatype declarations are similar to those in Standard ML,
with the main difference being syntactic,
where Standard ML's \verb,datatype, is instead \verb,data, in Haskell.
Haskell also provides record syntax for creating new types, allowing components
to be named.

%Type classes
Haskell's type classes allow for function overloading.
...
An example of a datatype that is an instance of a type class is the \verb,Lit, datatype
...
%List syntax
\paragraph{Lists}{
List types in Haskell are denoted using square brackets, e.g.,
a list of \verb,Lit,s has type \verb,[Lit],.
Lists can be appended using the \verb,++, operator, and elements
can be prepended to lists using the \verb,:, operator.
For example, \verb.1:[2,3,4] ++ [5,6]. gives the Haskell equivalent to
\verb.1::[2,3,4] @ [5,6]. in Standard ML.
...
solver to allow for better comparison of this project's model checker
...
for this project's model checker removes the
...
for the code \cite{haddock}. Haddock automatically generates documentation in
...
for unit testing in Java \cite{hunit}.
...
\cite{cabal}, where a package may be a library or a complete piece of software.
...
posed in conjunctive normal form \cite{minisat,een05}. Further details are given in section \ref{prep:minisat}.
}
...
The the binary version's assumptions on component ordering allow the format
...
examples from HWMCC'14 using the new version. The AIGER parser component
...
%Initiation query
The initiation query $I \Rightarrow P$ on line \ref{initiation} checks that the safety
...
I implemented several variants of the model checker component that differ
...
number of inputs. It also stores as a list of literals the outputs, bad states,
...
The wrapper function \verb,solveWithAssumps, for the version of \verb,solve(),
...
iff $\neg P \wedge I$ is unsatisfiable, and it uses deMorgan's laws (as described in
section \ref{prep:logic}) to acquire the asumption cube by negating all the literals in
...
(but limited in number of attempts) {\it mic} algorithm with the {\it down} function modified
to handle CTGs.
...
fulfilled as the {\it priorityQueue} implementation does,
