...
domains, such as model checking in the semi-conductor industry.
...
\subsection{Inductive Generalization}
Finding the minimal inductive subclause (MIC) of a clause is in practice inefficient
\cite{griggio14},
...
The modified {\it down} algorithm (Figure \ref{ctgdown})
checks, as in the simple approximation for MIC, for the satisfiability of
...
overview of the structure of the implementation for this variant of the algorithm,
...
The variant of the algorithm used in the {\it PriorityQueue} implementation (see Figure \ref{pqueuestr})
...
$(s, i + 1)$ is added to the queue by {\it pushFrame} (line \label{pqueuestr:enqueue}).
...
then {\it pushFrame} is called (line \label{pqueuestr:pushFrame}), and if not, a CTI is discovered with the intent
...
for proving the negation of the CTI could enqueued before
