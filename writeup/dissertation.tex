...
modules, which allows library functions to be used. Modules can be referred
...
The AIGER format has both an ASCII and a binary version; either
...
the process of finding the new CTI state(s) $d$ and trying to
...
If only one of the inputs ($i_1$ and $i_2$) is a Boolean, then if it is {\it False},
the singleton clause $\{\neg a\}$ is generated. Otherwise,
the clauses equivalent to $a \Leftrightarrow i$
...
I describe the overall structure shared by all the variants except
...
In the case that the consecution query (line \ref{recstr:consecution}) succeeds,
