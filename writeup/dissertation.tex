...
is usually called, taking a solver \verb,s, and a list of literals \verb,assumps,
...
%the C wrapper and C++ implementation of MiniSat. For example, the \verb,Solver, Haskell
%datatype is an
...
%analogously to the C wrapper's \verb,solveMinisatWithAssumps,,
%returning a \verb,Result, that contains
...
\tikzstyle{label}=[font=\footnotesize,scale=0.8]
\begin{tikzpicture}[auto, scale = 0.6]
  \node (haskell) [function] {solveWithAssumps s assumps {\it (Haskell MiniSat Interface)}};
  \node (haskellffi) [function, below of = haskell]
    {solveMinisatWithAssumps sol veclit {\it (Haskell FFI)}}
    edge [<-] (haskell);
  \node (wrapper) [function, below of=haskellffi]
    {solveMinisatWithAssumps (solver, assumps)
      {\it (C wrapper)}}
    edge [<-] (haskellffi);
  \node (minisat) [function, below of=wrapper]
    {solver -> solve(*assumps) {\it (MiniSat)}}
    edge [<-] (wrapper);
  \node (wrapper') [function, right = 4em of wrapper]
    {return \&res {\it (C wrapper)}}
    edge [<-] (minisat);
  \node (haskellffi') [function, above of = wrapper']
    {peek {\it (Haskell FFI)}}
    edge [<-] (wrapper');
  \node (haskell') [function, above of = haskellffi']
    {unsafePerformIO res {\it (Haskell MiniSat Interface)}}
    edge [<-] (haskellffi');
\end{tikzpicture}
...
on fourteen handwritten examples and 150 examples taken from
...
The variants could solve all the handwritten examples and fifty HWMCC examples within
...
\caption{Average literals per clause averaged over the 14 handwritten examples and fifty Hardware
...
from the initial state. Dealing with a set of states rather than a single CTI state
...
on examples, even in cases where \emph{CTG} reduces the average number of frames per clause (see Figures \ref{eval:time}, \ref{eval:litspercls}),
most likely because the examples used are too small for the performance benefits of using CTGs to
eliminate more states to overcome the overhead of finding and proving negated CTGs, which require
...
The reference implementation has a different hardware model representation than
...
to an ordering that was found to be the best choice empirically \cite{minisat}.
