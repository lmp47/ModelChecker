...
I implemented the basic IC3 algorithm as part of a model checker,
which involved implementing several additional components: the AIGER parser,
MiniSat interface, hardware model representation.
As an extension, I implemented other variants of the algorithm.
I benchmarked the variants on fourteen handwritten examples and roughly
fifty examples from the Hardware
Model Checking Competition and compared the results with those for
the IC3 reference implementation. The implementation of
the basic algorithm and its ability to solve
the handwritten examples meets the project's goals;
the implementation of other variants and the variants' ability to solve
...
\paragraph{Functions}{
Unlike in Standard ML, there is no keyword in Haskell for defining functions. Otherwise,
...
%Type classes
Haskell's type classes allow for ad hoc polymorphism.
...
Monads can be created with the function
...
The \verb,>>=, infix operator allows computations to be composed.
As expected given its type signature
\verb,(>>=) :: Monad m => m a -> (a -> m b) -> m b,, the function
takes a \verb,Monad, containing a computation that produces a value of type
\verb,a, and a function that takes values of type \verb,a, and returns
monads, and returns the result of applying the function to the value.
The \verb,>>, infix operator also allows computations to be composed,
...
in the form of \verb,do,-notation, which allows computations
inside \verb,Monad,s to be written in a more imperative fashion.
...
The model checker requires a way of taking input models and, since
...
(discussed in Section \cite{prep:logic}), the implementation also requires a representation of
