...
I chose the AIGER format for representing the hardware models and
...
Using Cabal enables the project to be built easily on different platforms,
...
Given logical formula $B$, a \emph{$B$ state} is a state contained in the set of states
represented
...
representing literals. MiniSat represents sets of clauses as \verb,vec<Lit>,s,
...
If there has been at least one query made of the \verb,Solver, object and the last query was
...
If $i_1 \wedge i_2$ holds, then singleton clause $\{a\}$ describes the constantly {\it True} AND gate,
and if not, then singleton clause $\{\neg a\}$ describes the constantly {\it False} AND gate.
...
In addition to the necessary language-related modifications to the algorithm, I 
changed how often the full propagation phase executes, calling the {\it pushFrame}
