...
Given a cube $c$, a $d$ is a i\emph{subcube} of $c$ (written $d \subset c$)
...
A boolean formula is in \emph{conjunctive normal form} (CNF) if it is a conjunction 
...
\Fn{down(cls, i)}{
  \lIf{$\neg (I \Longrightarrow {\it cls})$}{\Return{False}}
  \lIf{$F_i \wedge {\it cls} \wedge T \Longrightarrow {\it cls}'$}{\Return{True}}
  $p$ := $F_i \wedge t$ state such that $F_i \wedge t \wedge p \Longrightarrow \neg t'$ \;
  {\it cls} := $cls \cap p$\;
  \Return{down(cls,i)} 
}
...
Counterexamples to induction are found by the \verb,negCTI, function, which
...
When a CTI $c$ for proof obligation $(s,i)$ is discovered, it is sufficient to 
check that $I \Rightarrow \neg c$ holds and enqueue proof obligation $(c,1)$ before
calling {\it fulfillObligations} recursively, since then the proof obligation
$(c,i - 1)$ must be fulfilled before attempting to fulfill proof obligation $(s,i)$
again.
...
found, then the algorithm can terminate. Otherwise, the proof obligation
$(c,j)$ is enqueued, and {\it fulfillObligations} calls itself recursively.
