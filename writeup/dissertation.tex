...
Before explaining the algorithm, I provide an explanation of transition
systems and boolean representations of states.
...
where $x'$ is the set of next-state variables. A variable can take on
boolean values {\it True} or {\it False}.
...
The concepts of a literal, cube, and clause are useful in describing the
...
\Fn{prove$((i,x,I,T),P)$}{
  ...
  \While{True}{
    \eIf{$F_k \wedge T \Rightarrow P'$}{
      ...
      \While {$\neg (F_k \wedge T \Rightarrow P'$)}{
        {\rm cti} := {\it getCTI}($F_k \wedge T \Rightarrow P'$) \;
        \lIf{{\it proveNegCTI}$((i,x,I,T),{\rm cti},k - 1)$}{
        $F_k$ := $F_k \cup \{\neg{\rm cti}\}$}
        \lElse{\Return{False}}
      }
    }
