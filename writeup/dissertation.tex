...
I chose the AIGER format for representing hardware models and
...
Since model checking algorithm deals with transition systems
...
expansion of the macros in a \verb,.hsc, file produces a
...
is specified through the satisfying assignment of all state variables in the transition system
to Boolean values. A \emph{complete} assignment is represented as a cube such
...
with MiniSat variable $(2 \times x) + 1$. The \verb,model, returned in the \verb,result, struct
