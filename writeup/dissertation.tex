...
which checks CTL formulas \cite{hassan12}, makes use of both a
...
\paragraph{Types}{
Similarly to Standard ML, Haskell compilers perform type inference. Though
...
with the main difference being that Standard ML's \verb,datatype, is instead \verb,data, in Haskell.

Haskell provides record syntax for creating new types, allowing components
to be named.

%Type classes
Haskell also has type classes, which allow for function overloading.
Functions may be defined within the definition of a type class,
and will work for any type that is an instance of that type class.

Haskell's \verb,Monad, class encompasses composable
structures that describe computations. The described computations
...
%List syntax
\paragraph{Lists}{
The code written for this project makes extensive use
of lists. List types in Haskell are denoted using square brackets, e.g.,
a list of \verb,Lit,s has type \verb,[Lit],.
Lists can be appended using the \verb,++, infix operator, and elements
can be prepended to lists using the operator \verb,:,. For
...
Several tools provided in Aiger Utilities were used in the project
\cite{aiger}. The
...
are ignored by the parser, as they are not used by any of the model checker implementations.
...
\end{lstlisting}
\caption{The {\tt Lit} data structure in {\tt Parser.AigModel}}
\end{figure}
...
all the results of a SAT query. The struct contains not only the result of the SAT
