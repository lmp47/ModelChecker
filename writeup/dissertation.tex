...
\Fn{prove$((i,x,I,T),P)$}{
  ...
  \While{True \label{main}}{
    ...
    {\While {$\neg (F_k \wedge T \Rightarrow P')$ \label{reconsec}}{
        cti := {\it getCTI}($F_k \wedge T \Rightarrow P'$) \label{CTI} \;
        \lIf{{\it proveNegCTI}$((i,x,I,T),{\rm cti},k - 1)$ \label{negCTI}}{$F_k$ := $F_k \cup \{\neg{\rm cti}\}$}
...
The call to {\it getCTI} in line \ref{CTI} finds the counterexample to
...
will be called recursively with $k = 0$ and the attempt to show that
$\neg s$ is relatively inductive to $F_k$ fails.
...
Because there may be several counterxamples to induction, it is necessary to
...
An obvious choice for $c$ would be $c = s$, which can be found easily using
a SAT solver. In practice, it is better to generalize $s$ and choose
a smaller subset $c \subset s$, which can allow for a set of several states
to be eliminated in $F_k$ rather than just eliminating the single state $s$
when $\neg c$ is shown to hold at depth $k$.
In particular, the choice of $c$ that
...
Checking if a subclause $c = s \setminus \{l\}$ of a clause $s$ is inductive
...
has different implementations based on whether the version is recursive or not, but
is anal

In other words, the \verb,prove, function implements the first three lines of
pseudocode in the overview of the IC3 algorithm before calling the \verb,prove',
function, which implements a variation of the main while loop of the algorithm
on line \ref{main} of the pseudocode.
